...
function updateInventory() {
    ...
    Object.keys(inventory).forEach(block => {
        ...
        blockElement.textContent = inventory[block];
...
canvas.addEventListener('touchstart', (e) => {
    ...
    const blockType = blocks[Math.floor(y / blockSize)];
    if (blockType !== 'Sky') {
        blocks[Math.floor(y / blockSize)] = 'Sky';
        ...
        if (inventoryKeys.length > 0) {
            ...
            blocks[Math.floor(y / blockSize)] = blockToPlace;
